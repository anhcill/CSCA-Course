...
// GET /api/notifications — database-only; an empty inbox is a valid state.
router.get("/", protectRoute, async (req, res) => {
  try {
    ...
    const type = typeof req.query.type === "string" && req.query.type.trim()
      ? req.query.type.trim().slice(0, 50)
      : null;
...
// PATCH /api/notifications/:id/read — ownership is enforced by user_id.
...
// POST /api/notifications/read-all — only the current user's notifications.
