...
export default function VideoUploader({ onVideoUploaded }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-white space-y-6">
      ...
      {/* Video Title Input */}
      <div>
        <label className="block text-xs font-semibold uppercase text-slate-400 mb-2">Tiêu Đề Video</label>
        <input
          ...
          className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500 text-white"
        />
      </div>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-8 text-center transition ${
          file ? "border-rose-500/50 bg-rose-500/5" : "border-slate-800 bg-slate-950 hover:border-slate-700"
        }`}
      >
        <input
          id="video-file-input"
        ...
        {file ? (
          <div className="space-y-2">
            ...
            <p className="font-semibold text-rose-400">{file.name}</p>
          ...
          <label htmlFor="video-file-input" className="cursor-pointer space-y-2 block">
            ...
            <p className="font-medium text-slate-300">Kéo thả file video vào đây hoặc <span className="text-rose-400 underline">duyệt chọn file</span></p>
      ...
      {uploading && (
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-semibold">
            <span className="text-slate-400">Đang tải lên Cloudflare R2...</span>
