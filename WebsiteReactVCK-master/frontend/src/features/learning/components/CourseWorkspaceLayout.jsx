    ...
    <div className="min-h-full bg-[#f6f9fd] dark:bg-slate-950 px-4 py-6 sm:px-6 lg:px-8 transition-colors duration-200">
      <div className="mx-auto max-w-7xl space-y-5">
        <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400">
          <Link to="/lms/catalog" className="hover:text-blue-700 dark:hover:text-sky-400">Khóa học của tôi</Link>
          <span>/</span>
          ...
          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div className="min-w-0">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
                <span className="rounded-full bg-white dark:bg-slate-800 px-3 py-1.5 text-blue-700 dark:text-sky-300 shadow-sm ring-1 ring-blue-100 dark:ring-slate-700">
                  Lớp: {selectedClass.title || "Đang cập nhật"}
                </span>
                {course.category && (
          ...
          {workspaceItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) => `inline-flex shrink-0 items-center gap-2 rounded-xl px-3 py-2.5 text-xs font-bold transition ${
                  isActive
