  ...
  return (
    <div className="min-h-full bg-[#f6f9fd] px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {loading ? <LoadingState variant="light" message="Đang tải tài liệu..." count={3} /> : errorMessage ? <ErrorState variant="light" title="Không tải được tài liệu" message={errorMessage} onRetry={loadFiles} /> : visibleFiles.length === 0 ? <EmptyState variant="light" icon={FolderOpen} title="Chưa có tài liệu phù hợp" description="Tài liệu giáo viên chia sẻ sẽ xuất hiện tại đây." /> : <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visibleFiles.map((file) => {
