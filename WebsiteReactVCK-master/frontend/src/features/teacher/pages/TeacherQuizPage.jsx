    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-8 px-4 sm:px-6 lg:px-8 pb-20 font-sans">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {quizzes.map((quiz) => (
            <article
              key={quiz.id}
              className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 hover:border-amber-500/40 rounded-3xl p-6 space-y-5 transition flex flex-col justify-between group shadow-sm"
            >
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">{quiz.courseTitle}</p>
              </div>

              <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-white/5">
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2.5 border border-slate-100 dark:border-transparent">
                    <span className="text-slate-500 text-[10px] uppercase font-bold block">Câu hỏi</span>
                    <span className="font-mono text-slate-900 dark:text-white font-bold text-sm mt-0.5 block">{quiz.questionCount}</span>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2.5 border border-slate-100 dark:border-transparent">
                    <span className="text-slate-500 text-[10px] uppercase font-bold block">Thời gian</span>
                    <span className="font-mono text-slate-900 dark:text-white font-bold text-sm mt-0.5 block">{quiz.timeLimitMinutes}p</span>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-2.5 border border-slate-100 dark:border-transparent">
                    <span className="text-slate-500 text-[10px] uppercase font-bold block">Lượt nộp</span>
                    <span className="font-mono text-amber-600 dark:text-amber-400 font-bold text-sm mt-0.5 block">{quiz.attemptsCount}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white text-xs font-bold transition flex items-center justify-center gap-1.5"
                  >
      ...
      {isBuilderOpen && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4 animate-fade-in font-sans">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl sm:rounded-3xl max-w-3xl w-full shadow-2xl relative flex flex-col max-h-[88vh] overflow-hidden">
            {/* Fixed Header */}
            <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-white/95 dark:bg-gray-900/95 backdrop-blur">
              ...
              <button
                ...
                className="p-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 hover:text-white transition"
              >
            ...
            {/* Fixed Footer */}
            <div className="shrink-0 px-6 py-3.5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/90 dark:bg-gray-900/95 backdrop-blur flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setIsBuilderOpen(false)}
                className="px-4 py-2 rounded-xl bg-gray-200 dark:bg-gray-800 text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700 transition"
              >
                Hủy bỏ
              </button>
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold shadow-md shadow-amber-600/20 transition"
              >
