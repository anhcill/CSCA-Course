  ...
  return (
    <div className="min-h-full bg-slate-50 px-4 py-6 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {activeTab === "tasks" && <TeacherSessionTasks tasks={tasks} onCreate={() => setIsCreateAssignmentOpen(true)} />}
      ...
      {isCreateAssignmentOpen && <CreateAssignmentModal isOpen classId={classId} sessionId={sessionId} sessionTitle={session.title} onClose={() => setIsCreateAssignmentOpen(false)} onCreated={loadData} />}
    </div>
  );
}
...
function TeacherSessionTasks({ tasks, onCreate }) { return <section className="space-y-4"><div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div><h2 className="font-black">Bài tập & Quiz của buổi</h2><p className="mt-1 text-xs text-slate-500">Chỉ học viên của lớp này mới thấy các nhiệm vụ này.</p></div><button type="button" onClick={onCreate} className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white">Giao bài / tạo Quiz</button></div>{tasks.length ? tasks.map((task) => <article key={`${task.type}-${task.id}`} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div><span className="rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-black text-violet-700 dark:bg-violet-950/60 dark:text-violet-300">{task.type === "quiz" ? "QUIZ" : "BÀI TẬP"}</span><h3 className="mt-2 text-sm font-black">{task.title}</h3><p className="mt-1 text-xs text-slate-500">Đã nộp: {task.submittedCount || 0}/{task.totalCount || 0} · Cần chấm: {task.pendingGradingCount || 0}</p></div><span className="text-xs font-bold text-slate-500">{task.type === "quiz" ? "Chấm tự động" : "Theo dõi nộp bài"}</span></article>) : <EmptyState icon={ClipboardList} title="Chưa có nhiệm vụ" description="Giao bài tập hoặc tạo Quiz ngay trong buổi học này." />}</section>; }
